...
est2 <- merge(est[year==2019, .(iso3,pop,g.whoregion,g.income,inc,inc.sd)], tb[year==2019, .(iso3,ep)], by='iso3')
...
bpos <- est3[, {
  tmp = prodXY(inc * (1 - ep), hic, (inc.sd * (1 - ep)) ^ 2, hic.sd ^ 2)
  
  list (inc.bc = tmp[[1]],
        inc.bc.sd = sqrt(tmp[[2]]))
},
by = .(iso3)]
...
save(bpos, file = here('Rdata/bpos.Rdata'))
