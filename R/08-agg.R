...
#' add tbhiv
#'
out <- global[, divXY(inc.h, inc, inc.h.sd ^ 2, inc.sd ^ 2)]
out2 <- vlohi(out[[1]], sqrt(out[[2]]))
global$tbhiv <- out[[1]]
global$tbhiv.lo <- out2[1,]
global$tbhiv.hi <- out2[2,]
global[, tbhiv.sd := sqrt(out[[2]])]
...
#' add CFR
#'
out <- global[, divXY(mort, inc, mort.sd ^ 2, inc.sd ^ 2)]
out2 <- vlohi(out[[1]], sqrt(out[[2]]))
global$cfr <- out[[1]]
global$cfr.lo <- out2[1,]
global$cfr.hi <- out2[2,]
global[, cfr.sd := sqrt(out[[2]])]


save(global, file = here('Rdata/global.Rdata'))
...
#' add CFR
#'
out <- regional[, divXY(mort, inc, mort.sd ^ 2, inc.sd ^ 2)]
out2 <- vlohi(out[[1]], sqrt(out[[2]]))
regional$cfr <- out[[1]]
regional$cfr.lo <- out2[1,]
regional$cfr.hi <- out2[2,]
regional[, cfr.sd := sqrt(out[[2]])]

save(regional, file = here('Rdata/regional.Rdata'))
...
save(hbc, file = here('Rdata/hbc.Rdata'))
